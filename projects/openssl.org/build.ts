import { BuildOptions, run, unarchive, Range } from "brewkit";
...
export default async function ({ prefix, version }: BuildOptions) {
  ...
  if (new Range("^1").satisfies(version)) {
    switch (`${Deno.build.os}/${Deno.build.arch}`) {
      case 'darwin/aarch64': target = 'darwin64-arm64-cc'; break;
      case 'darwin/x86-64': target = 'darwin64-x86_64-cc'; break;
      case 'linux/aarch64': target = 'linux-aarch64'; break;
      case 'linux/x86-64': target = 'linux-x86_64'; break;
    default:
