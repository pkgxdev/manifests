import { BuildOptions, Path, env_include, inreplace, insert, run, unarchive, undent } from "brewkit";
...
function change_ruby_function(file: string, function_name: string, body: string | ((foo: string) => string)) {
  ...
  const start = lines.findIndex((line) =>
    new RegExp(`^\\s*def\\s+${function_name}`).test(line)
  );
  ...
  const end = lines.slice(start).findIndex((line) => line.trim() == "end") + start;
