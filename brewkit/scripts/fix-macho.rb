...
def fix_install_names
  $file.linked_dylibs.map do |lib|
    ...
    if lib.start_with? $prefix
      rel_path = Pathname.new($file.filename).relative_path_from($final_prefix)
      rel_path = Pathname.new($prefix).join(rel_path)
      rel_path = Pathname.new(lib).relative_path_from(rel_path.parent)
