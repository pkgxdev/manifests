...
export default async function default_fixups(
  ...
  flatmap(
    install_prefix.join("man").isDirectory(),
    (mandir) => mandir.mv({ into: install_prefix.join("share").mkdir() }),
  );

  let walker = walk(final_prefix.string, {
    includeFiles: true,
    includeDirs: true,
    includeSymlinks: true,
  });

  const rpather = new RpathFixer(install_prefix, final_prefix, dep_prefixes, PKGX_DIR);
    ...
    if (isSymlink) {
      ...
      switch (await mime(path)) {
        ...
        default:
          switch (path.extname()) {
            case ".pc":
              await fix_pkg_config_file(path, install_prefix);
              break;
            case ".pyc":
            case ".la":
            case ".a": // static libs are not useful to our purposes
              ...
              continue;
            case "cmake":
              await fix_cmake_file(path, install_prefix);
              break;
          }
      }
  ...
  install_prefix.join("share/doc").rm("rf");
