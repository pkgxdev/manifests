...
export default class Path {
  ...
  static cwd(): Path {
    return new Path(Deno.env.get("PWD")!);
  }
  ...
  constructor(input: string | Path) {
    ...
    function normalize(path: string): string {
      ...
      const start = Deno.build.os == "windows"
        ? (segments.shift() || "\\") + "\\"
        : "/";
  ...
  ln(_: "s", opts: { target: Path } | { string: string }): Path {
    if ("target" in opts) {
      Deno.symlinkSync(opts.target.string, this.string);
    } else {
      Deno.symlinkSync(this.string, opts.string);
    }
    return this;
  }
  ...
  write(
    { force, ...content }:
      & ({ text: string } | { json: PlainObject; space?: number })
      & { force?: boolean },
  ): Path {
    if (this.exists()) {
